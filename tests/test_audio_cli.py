...
def test_seek_rejects_disallowed_absolute_output_path(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 1
    assert "Output path is restricted" in result.output


def test_seek_rejects_relative_output_path_traversal(tmp_path: Path) -> None:
    prepared_root = tmp_path / "prepared"
    index_path = prepared_root / "GEN" / "001.index.json"
    wav_path = prepared_root / "GEN" / "001.wav"
    index_path.parent.mkdir(parents=True, exist_ok=True)
...
def test_seek_rejects_index_wav_path_outside_prepared_root(tmp_path: Path) -> None:
    prepared_root = tmp_path / "prepared"
    index_path = prepared_root / "GEN" / "001.index.json"
    index_path.parent.mkdir(parents=True, exist_ok=True)
    index_path.write_text(
        '{"sample_rate":16000,"total_samples":32000,"duration_sec":2.0,'
        '"wav_path":"/etc/passwd","book_usx":"GEN","chapter":1,'
        '"created_at":"2026-05-29T00:00:00Z"}'
    )
    ...
    assert result.exit_code == 1
    assert "wav_path must be under prepared root" in result.output
